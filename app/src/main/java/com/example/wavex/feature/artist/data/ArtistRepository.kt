package com.example.wavex.feature.artist.data
...
class ArtistRepository @Inject constructor(
    ...
    private fun getFavouriteArtistReference(artistId: String): DatabaseReference {
    ...
    suspend fun isFavouriteArtist(artistId: String): Boolean {
        return getFavouriteArtistReference(artistId)
            .get()
            .await()
            .exists()
    }

    suspend fun toggleFavouriteArtist(
        artistId: String,
        artistName: String,
        artistImageUrl: String,
        source: String
    ): Pair<Boolean, String> {
        val reference = getFavouriteArtistReference(artistId)
        ...
        return if (!snapshot.exists()) {
            ...
            reference.setValue(artistData).await()

            Pair(true, "Added To Favourite")
        } else {
            reference.removeValue().await()
            Pair(false, "Removed From Favourite")
        }
    }
    ...
    suspend fun fetchYTMusicArtist(artistId: String,baseUrl: String): ArtistDetailUiState =
        withContext(Dispatchers.IO) {
            try {
